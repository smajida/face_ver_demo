...
bool faceVerfication(LightFaceRecognizer& recognizer, FaceAlign& face_align, fs::path &path, VideoCapture & cap)
{
  // Parameter settings.
  ...
  if (0 >= all_image_path.size()) {
    cerr<<"The person with name "<<path.filename()<<" does not exist!"<<endl;
    return false;
  }
  ...
  // Extract face feature
    Mat face = face_align.align(frame, face_rect, 224, FaceAlign::INNER_EYES_AND_BOTTOM_LIP, 0.3);
    Mat f;
    ...
    if (is_start && *min_iter < dist_threshold) {
      ...
      putText(frame, buf, Point(frame.cols - 250, 50), CV_FONT_HERSHEY_COMPLEX, 1, Scalar(255, 0, 0));
    }
    ...
    putText(frame, buf, Point(20, 50), CV_FONT_HERSHEY_COMPLEX, 1, Scalar(255, 0, 0));
    string result = num_accepted==num_min_accept_frame ? "SUCCESS" : 
      (i==num_max_check_frame ? "FAIL" : "") ;
    putText(frame, result, Point(frame.cols/2, 100), CV_FONT_HERSHEY_COMPLEX, 1, Scalar(255, 0, 0));
    imshow(winName, frame);

    // Wait key.
    char key;
    if ( is_start && (num_accepted == num_min_accept_frame || i ==num_max_check_frame) ) {
      key = (char) waitKey(-1);
...
bool faceRegister(LightFaceRecognizer& recognizer, FaceAlign& face_align, fs::path &path, VideoCapture & cap)
{
  ...
  for (;;) {
    ...
    switch ( key ) {
      case 27: 
        return false;
      case ' ':
        Mat face = face_align.align(origin, face_rect, 224, FaceAlign::INNER_EYES_AND_BOTTOM_LIP, 0.3);
...
int main(int argc, char **argv) {
  ...
  LightFaceRecognizer recognizer(
      "../../models/cnn",
      "../../models/bayesian_model_lfw.bin", 
      "prob", false);
  FaceAlign face_align("../../models/dlib_shape_predictor_68_face_landmarks.dat");
